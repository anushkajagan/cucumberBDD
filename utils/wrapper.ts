...
export class PlaywrightWrapper {
  ...
  async click(selector: string): Promise<void> {
    ...
    await this.page.waitForLoadState('networkidle', { timeout: DEFAULT_TIMEOUT });
  }
  ...
  async getRowsValue(selector: string): Promise<string[]>{
    const table = this.page.locator(selector);
    //const table = this.page.locator("table[class*='MuiTable-root']");
    await table.waitFor({ state: 'attached', timeout: 10000 });
    const count1 = await table.count();

if (count1 > 0) {
  logger.info("Table found on the page");
} else {
  logger.warn("Table NOT found on the page");
}
    const count = await table.locator('a').count();
    logger.info(`Total text link in the table1: ${count}`);
    const rowText: string[] = [];
    for (let i =0;i<count;i++){
      rowText[i] = await table.locator('a').nth(i).innerText({ timeout: DEFAULT_TIMEOUT });
      //rowTexts.push(rowText);
      logger.info(`Row ${i+1} : ${rowText[i]}`); 
      
    }
   return rowText;
  }

  async getAltText(selector: string): Promise<void> {
    const altText = this.page.getByAltText(selector);
    expect(altText).toBeVisible({ timeout: DEFAULT_TIMEOUT });
    logger.info(`Alt text ${selector} is visible`);
  }

  async ClickOnTableLink(Selector: string, Certification : string ): Promise<void>{
    const table = this.page.locator(Selector);
    await table.waitFor({ state: 'attached', timeout: 10000 });
    const certNameCount = await table.locator('a').count();
    const linkCount = await table.locator('button').count();
    logger.info(`Total text link in the table: ${certNameCount}`);
    logger.info(`Total buttons in the table: ${linkCount}`);

    if (certNameCount && linkCount > 0 ){

      for (let i =0;i<certNameCount;i++)
      {
        const certName = await table.locator('a').nth(i).innerText({ timeout: DEFAULT_TIMEOUT });
        console.log(`Certification Name ${i+1} : ${certName}`);
        if (certName.includes(Certification)) 
          {
            await table.locator('button').nth(i).click();
            logger.info(`Clicked on ${Certification} link`);
            await table.waitFor({ state: 'detached', timeout: 60000 });
            break; // Exit the loop after clicking the desired certification link
          }else if(i==certNameCount-1){
            logger.warn(`Certification ${Certification} not found in the table`);
          }
      }

    }else{
      logger.warn(`No links or buttons found in the table`);
    }
  }
}